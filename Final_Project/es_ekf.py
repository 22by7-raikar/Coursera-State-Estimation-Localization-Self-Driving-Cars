...
################################################################################################
with open('data/pt1_data.pkl', 'rb') as file: #Loaded the file
    data = pickle.load(file)

# with open('data/pt2_data.pkl', 'rb') as file: #Loaded the file
#     data = pickle.load(file)

# with open('data/pt3_data.pkl', 'rb') as file: #Loaded the file
#     data = pickle.load(file)
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    M = np.eye(3)
    K = p_cov_check @ h_jac.T @ np.linalg.inv((h_jac @ p_cov_check @ h_jac.T) + (M @ sensor_var @ M.T))
    ...
    v_hat += (K @ error[3:6])
    q_error = K @ error[6:]
    q_hat += Quaternion(euler=q_error).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    q_new = Quaternion(euler=(imu_w.data[k - 1]*delta_t)) 

    # 1. Update state with IMU inputs
    p_check = p_est[k-1] + (v_est[k-1] * delta_t) + 0.5*f_ns*(math.pow(delta_t, 2))
    v_check = v_est[k-1] + (f_ns*delta_t)
    q_check = q_new.quat_mult_right(Quaternion(euler=(imu_w.data[k - 1]*delta_t)))  #Check Formula ################
    ...
    p_cov_hat = (F @ p_cov_hat @ F.T) + (l_jac @ Q @ l_jac.T)
